...
class MyDES:
    ...
    def _bit_decode(s: list):
        ...
        for item in s:
            result_string += item
        binary_int = int(result_string, 2)
        byte_number = binary_int.bit_length() + 7 // 8
        binary_array = binary_int.to_bytes(byte_number, "big")
        ascii_text = binary_array.decode().strip("\x00")
        return ascii_text
    ...
    # здесь происходит преобразование 64-битной строки в двоичную
    @staticmethod
    def _replace_block(block: str, replace_table: tuple) -> str:
        ...
        """
        result = ""
        print("\nПреобразование 64-битной строки в двоичную: ")
    ...
    def _processing_encode_input(self, enter: str) -> list:
        ...
        print(f"\nИсходный открытый текст преобразован в двоичный:\n {result}")
        return result
    ...
    def _spin_key(self, key: str):
        ...
        print("\nПреобразование 64-битного исходного ключа в 56-битный ключ и выполнение замены\n"
              "а также осуществление сдвига для получения ключа:")
        ...
        for i in range(1, 17):
            ...
            print(f"\nКлюч после сдвига: слева: {first_after_spin}, справа: {second_after_spin}")
    ...
    def _not_or(a: str, b: str) -> str:
        """
             XOR двух строк 01
        """
        print(f"\nОперация XOR двух строк {a} и {b}: ")
        result = ""
        size = len(a) if len(a) < len(a) else len(b)
        for i in range(size):
            result += '0' if a[i] == b[i] else '1'
        print(f'Результат опреации XOR: {result}')
        return result
    ...
    def _s_box_replace(block48: str) -> str:
        ...
        for i in range(8):
            ...
            row = int(row_bit, 2)
            line = int(line_bit, 2)
            # print(f"Строка {line}, столбец {row}")
    ...
    def _s_box_compression(self, num: int, block48: str) -> str:
        """
             После расширения и замены сжатие S-блока выполняется на 48-битной строке 01.
        ...
        """
        print("\nВыполняется сжатие S-блока выполняется для 48-битной строки"
              " после расширения и замены:")
    ...
    def _p_box_replacement(self, block32: str) -> str:
        ...
        """
        print("\nПолучение 32-битной строки после замены P блока: ")
        p_box_replace_table = (
    ...
    def _f_function(self, right: str, is_decode: bool, num: int):
        ...
        print(f"Результат замены блока s: {sbc_result}")
    ...
    def encode(self, enter: str, key: str):
        ...
        for block in blocks:
            ...
            print(f"Шифр после окончательной замены состояния: {block_result}")
    ...
    def decode(self, cipher_text: str, key: str):
        ...
        for block in blocks:
            ...
            block_result = self._end_replace_block(block_result)
            print(f"\nШифр после окончательной замены состояния: {block_result}")
...
if __name__ == '__main__':
    key = input("Введите ключ для шифра DES: ")
    md = MyDES()
    start_word = input("Введите исходное слово для шифра DES: ")
    print(f"Исходное слово: {start_word}")
    print(f"Ключ: {key}")
    result = md.encode(start_word, key)
    print("Зашифрованные данные DES: " + result)
    print("=====================================================================")
    print("Расшифрованные данные: " + md.decode(result, key))
    print("=====================================================================")
