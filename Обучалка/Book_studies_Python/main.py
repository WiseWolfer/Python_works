...
# Операции с экземплярами через индекс
# Класс
# class MyClass:
#     # Поле класса
#     Nmax = 5
...
# # содержимое списка
# print(obj)
# # новые значения элемента списка
# obj[0] = 100
# obj[2] = -3
# obj[24] = 123
# # содержимое списка
# print(obj)
# # считывание значений элемента списка
# print("Элемент с индексом 0:", obj[0])
# print("Элемент с индексом 1:", obj[1])
# print("Элемент с индексом 2:", obj[2])
# print("Элемент с индексом 3:", obj[3])
...
#         print("Метод __getattribute__() завершил свою работу.")
#         return res
#     # метод вызывается, если поле удаляется
#     def __delattr__(self, attr):
...
#         # экземпляр класса
#         return tmp
#    # метод позволяет к числу прибавить экземпляр
#     def __radd__(self, x):
#         return self + x
...
# b = a + 5
# c = 10 + b
# # проверяем значение первого экземпляра a
# print(a)
# # проверяем значение первого экземпляра b
# print(b)
# # проверяем значение первого экземпляра с
# print(c)
...
#         self.y = y
#         self.z = z
#     # метод для присваивания к текстовому типу
#     def __str__(self):
#         # Текстовое значение
#         txt = "<" + str(self.x) + "|"
#         txt += str(self.y) + "|"
...
#         # создается экземпляр класса Vector
#         t = Vector()
#         t.x = self.x + obj.x
#         t.y = self.y + obj.y
#         t.z = self.z + obj.z
#         return t
#     # сложение вектора с присваиванием
...
#         return -obj + self
#     # разность вектора с присваиванием
...
# создать список из заданных слов и найти слово стартующие с буквы А
...
#     return names
#
# number = int(input(" pmuch "))
# names = make_list(number)
# for n in names:
#     if n.startswith("A"):  # метод проверяет начало с буквы A
#         print("name ",n, " in A")
...
# show_me()
# show_me(100)
# # Аргументы функции формируются на основе списка
# nums = [10, 55, 2+3j, 0.123]
# show_me(*nums)
# show_me("Python", "Java")
#
...
#     # кол-во необязательных аргументов
#     n = len(other)
#     print("Ещё осталось", n, "аргумента:")
...
# # Аннотация для аргумента a
# annt = show.__annotations__["a"]
# print("Аргумент a:", annt)
# # Аннотация для результата (записываем в res,
# # что возвращает функция (в данном случае None))
# res = show.__annotations__["return"]
# print("Возращаемый результат:", res)
#
# # Аннотации как команды
# # сначала выводится интерпретатором декларат. элементы в описании функции
# # потом уже делаются принты после функции и уже идет выполнение самой функции
...
#         # Кол-во элементов в списке или тексте
#         # При попытке вызывать функцию len() для целого
#         # числа возникает ошибка (некорректный тип данных)
#         print("Кол-во элементов:", len(names[n]))
#         # неверный индекс или деление на 0
#         names[n+1] //= n
...
#         print("Ошибка:", err)
#     # обработка ошибок, свзяанных с некорректным индексом
#     # или делением на 0
...
# print("Работа завершена.")
#
# # генерирования исключений (инструкция raise)
# # создаем экземпляр исключения
# err_one = ZeroDivisionError("Ошибка деления на 0!")
...
# использование инструкции assert
...
#         # добавляем описание для экземпляра исключения
#         # при приведении экземпляра к текстовому формату
#         # будет возвращаться написанная текстовая строка(смотри print() в блоке except)
#         assert arg, "Неожиданная ошибка"
...
#         # контролируемый код
#         try:
#             # Присваивание значений полю экземпляра.
#             # Возможно ошибка деления на 0
#             self.x = -c / b
#         # Если возникла ошибка деления на 0
#         except ZeroDivisionError:
...
#         # дискременант уравнения
#         D = b * b - 4 * a * c
#         # если дискременант меньше нуля
#         if D < 0:
#             # Генерируется исключение
#             # пользовательского типа
...
#         self.b = 1 # следующее генерируемое число
...
#             # поля count, a и b получают
#             # начальные единичные значения
#             self.start()
#             # генерируется исключение класса StopIteration
#             raise StopIteration
#         # если лимит генерирования
#         # чисел не превышен
#         res = self.a # возвращаемое методом значение
#         # нове число в последовательности Фибоначчи
#         self.a, self.b = self.b, self.a + self.b
#         # новое значения поля count
#         self.count += 1
#         # результат метода
#         return res
# # создается экземпляр класса
...
#     print(s, end=" ")
# print()
# # кол-во генерируемых чисел 15
# obj.n = 15
# # Ещё один оператор цикла с экземпляром класса
# for s in obj:
#     print(s, end=" ")
...
def fib_gen(n):
    # первое число последовательноси
    a = 1
    # второе число последовательноси
    b = 1
    # оператор цикла для вычисления чисел фибоначчи
    for i in range(n):
        ...
        a, b = b, a + b
        # результат функции генератора, добавляет результат в последовательность
        yield res
print("Попытка №1")
...
for s in fib_gen(10):
    print(s, end=" ")
print("\nПопытка №2")
# Ещё один рператор цикла для вывода
...
for s in f:
    print(s, end=" ")
print("\nПопытка №4")
# Еще один оператор цикла для вывода 15 чисел
# из последовательности Фибоначчи.
# Ссылка на объект генератора выполнена
# через переменную
for s in f:
...
lox = list(fib_gen(15))
print("Созданный список по последовательности Фибонначи:" + "\n" + str(lox))

# функция map(), filter(), zip()
# похволяют создавать одноразовые объекты итерационного типа
# это значит, что несколько раз в разных циклах мы их не можем использовать
# а можем использовать только единожды

# map (результат: степени двойки)
print(list(map(lambda n: 2**n, [1, 2, 3])))
# filter (результат: список нечетных элементов)
print(list(filter(lambda x: bool(x % 2), [1, 5, 6, 2, 7, 8])))
# zip получаем из списков - список кортежей, где
# каждый кортеж полчается включением соответствующих элементы списков
x = [1, 2, 3]
y = [4, 5, 6]
z = [7, 8, 9]
